		...
		<table class="table table-striped" border="1">
			...
			<tbody>
				<?php
					...
					$totalOngkir = 0;
					$no = 0;
					$q = mysql_query("Select booking.* from booking order by id desc") or die (mysql_error());
					while ($data = mysql_fetch_object($q)) {
						$totalHarga = 0;
						$no++;
						$q2 = mysql_query("Select detail_booking.*, event.harga from detail_booking INNER JOIN event ON detail_booking.event_id = event.id where booking_id = '$data->id'") or die (mysql_error());
