		...
		<?php
			if(!empty($_GET)){
				...
				if ($_GET['act'] == "verified") {
					...
					if ($updatePembayaran) {
						$qPembayaran = mysql_query("Select * from pembayaran where id_booking='$id_booking' and status='verified'") or die (mysql_error());
						$totalPembayaran = 0;
						while ($d = mysql_fetch_object($qPembayaran)) {
							$totalPembayaran += $d->total;
						}
						$q1 = mysql_query("select*from detail_booking where booking_id='$id_booking'");
						$totalBayar = 0;
						...
						$q3 = mysql_query("select*from booking where id='$id_booking'") or die (mysql_error());
						$booking = mysql_fetch_object($q3);

						$totalBayar += $booking->ongkir;
						if ($totalBayar <= $totalPembayaran) {
							$updatePesanan = mysql_query("update booking set status='lunas' where id='$id_booking'") or die (mysql_error());
							if ($updatePesanan) {
				...
				if ($_GET['act'] == "detail") {
					...
					$dataPesanan = mysql_fetch_object(mysql_query("Select*from booking where id='$dataPembayaran->id_booking'"))
					?>
					<div class="col-md-6">
									...
									<td>
										<?php
											if ($dataPembayaran->status == "pending") {
												?>
												<a href="pembayaran.php?act=verified&id=<?php echo $dataPembayaran->id; ?>&id_booking=<?php echo $dataPembayaran->id_booking; ?>" class="btn btn-sm btn-success">Verified</a>
												<?php
											}
										?>
									</td>
		...
		<?php while($data=mysql_fetch_object($q)){ ?> 
				<tr <?php if($data->status == "pending" ){ echo 'style="background:#cce9f8 !important;"'; } ?> > 
					...
					<td>
						...
						<a class="btn btn-sm btn-success" href="detail_booking.php?id=<?php echo $data->id_booking ?>">Booking</a>
					</td> 
				</tr>
		<?php } ?>
			</tbody> 
