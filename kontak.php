<?php 
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'create'){
		if(!empty($_POST)){
			...
			$q = mysql_query("insert into kontak Values(NULL,'$nama','$email','$subjek','$book')");
			if($q){ alert("Success"); redir("kontak.php"); }
		}
	}
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			...
			$q = mysql_query("update kontak SET nama='$nama',email='$email',subjek='$subjek',book='$book' where id=$_GET[id]") or die(mysql_error());
			if($q){ alert("Success"); redir("kontak.php"); }
		}
	}
